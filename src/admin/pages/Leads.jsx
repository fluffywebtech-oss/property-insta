...
export default function AdminLeads() {
  ...
  const exportCsv = () => {
    const cols = ['ref', 'name', 'phone', 'email', 'type', 'status', 'property_title', 'visit_date', 'visit_time', 'created_at', 'message'];
    const esc = (v) => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const rows = filtered.map(l => [l.ref, l.name, l.phone, l.email, typeOf(l), l.status || 'New', l.property_title, l.visit_date, l.visit_time, l.created_at, l.message].map(esc).join(','));
    ...
    <div className="adm-page">
      ...
      <div className="adm-table-wrap">
        <table className="adm-table">
          <thead><tr><th>Ref</th><th>Name</th><th>Type</th><th>Interest</th><th>Contact</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {filtered.map(l => (
              <tr key={l.ref}>
